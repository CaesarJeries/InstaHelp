...
public class PendingRequestsAdapter extends BaseAdapter{
    ...
    public PendingRequestsAdapter(Activity activity, String user_id){
    ...
    @Override
    public int getCount() {
        return gePostsWithMyHelpOffer().size();
    }

    @Override
    public Object getItem(int position) {
        List<Post> snapshot = new LinkedList<>();
        snapshot.addAll(gePostsWithMyHelpOffer().values());
    ...
    private Map<String, Post> gePostsWithMyHelpOffer(){
        Map<String, Post> retval = new HashMap<>();
        for(Post p : dataManager.getPosts().values()){
            if(p.getHelpOffers().containsValue(user_id)){
                retval.put(p.getID(), p);
            }
            if(p.getChosenUserId() != null){
                if(p.getChosenUserId().compareTo(user_id) == 0) retval.put(p.getID(), p);
            }
        }
        return retval;
    }
